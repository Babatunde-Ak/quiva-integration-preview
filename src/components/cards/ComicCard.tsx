...
interface ComicCardProps extends Readonly<Partial<Comic>> {
...
export function ComicCard({
  ...
  const displayCreator = creator || creatorUsername || (creatorWalletAddress 
    ? `${creatorWalletAddress.slice(0, 6)}...${creatorWalletAddress.slice(-4)}`
    : "Creator")
  
  // Use views from comic data or prop
  const displayViews = viewsFromComic ? formatNumberShorthand(viewsFromComic) : formatNumberShorthand(viewsProp)
      ...
      <div className="pt-3 sm:pt-4">
        ...
        <div className='flex justify-between items-center'>
          ...
          <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs text-white flex-shrink-0">
            <button 
              onClick={handleLike}
              className={`p-0.5 sm:p-1 transition-colors ${liked ? 'text-red-300' : 'text-white hover:text-red-300'} flex flex-col items-center`}
            >
              <Heart size={10} className="sm:w-3 sm:h-3" fill={liked ? 'currentColor' : 'none'} stroke={liked ? 'currentColor' : 'white'} />
            ...
            <div className='flex flex-col items-center'>
              ...
              <div className="flex items-center gap-0.5 mt-0.5">
                <span className="text-[9px] sm:text-[10px]">{price ?? currentPrice}</span>
